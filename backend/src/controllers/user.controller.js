...
export async function deleteSAddress(req, res) {
  try {
    const { addressId } = req.params;
    const user = req.user;
...
export async function addToWishlist(req, res) {
  try {
    ...
    if (!req.user) {
      console.log('❌ No user found - auth issue');
    ...
    // Check if product is already in the wishlist
    if (user.wishlist.includes(productId)) {  // ✅ Fixed typo
...
export async function removeFromWishlist(req, res) {
  try {
    ...
    console.log('🗑️ removeFromWishlist called');
    console.log('ProductId:', productId);
    ...
    if (!user.wishlist.includes(productId)) {  // ✅ Fixed typo
      console.log('⚠️ Product not in wishlist');
      return res.status(400).json({ error: "Product not found in wishlist" });
    }
...
export async function getWishlist(req, res) {
  try {
    ...
    console.log('✅ Wishlist retrieved:', user.wishlist.length, 'items');
